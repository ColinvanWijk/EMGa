...
from apps import tue_header, fig_day, buttons, score_info, login, feas_check, messages
...
layout = html.Div([
                ...
                [
                    ...
                    dbc.Col([
                        ...
                        html.Div([
                            ...
                            dbc.Tooltip(
                                "Figure presenting your submitted Bid, the Actual power generated by your portfolio, and the day-ahead Price.",
                                target="fig_1", placement='left-end', style={'font-size': '0.7vw'}
                            ),
                            dcc.Loading(fig_day.fig_bid(), color=app.color_3, type='circle'),

                        ], id='fig_1', style={'width': '40vw', 'height': '25vw'}),
                    ], width=5, lg=5, md=5),
                    ...
                    dbc.Col([
                        ...
                        html.Div([
                            dbc.Tooltip(
                                "Figure presenting the Imbalance with your submitted bid, the Feasible imbalance considering the characteristics of your portfolio,"
                                " and the positive and negative imbalance prices.",
                                target="fig_2", placement='left-end', style={'font-size': '0.7vw'}
                            ),
                            dcc.Loading(fig_day.fig_unbal(), color=app.color_3, type='circle'),
                        ], id='fig_2', style={'width': '40vw', 'height': '25vw'}),
                    ], width=5, lg=5, md=5),
            ...
            dbc.Row(
                [
                    ...
                    dbc.Col([
                        html.Div([
                            dbc.Tooltip(
                                "Figure presenting the Expected revenue with the imbalance of your submitted bid, the Feasible revenue"
                                "with the feasible imbalance, and the Perfect forecast revenue.",
                                target="fig_3", placement='left-end', style={'font-size': '0.7vw'}
                            ),
                            dcc.Loading(fig_day.fig_ahead(), color=app.color_3, type='circle'),
                        ], id='fig_3', style={'width': '40vw', 'height': '25vw'}),
                    ], width=5, lg=5, md=5),
                    ...
                    dbc.Col([
                        html.Div([
                            ...
                            dbc.Tooltip(
                                "Figure presenting the accumulated revenue considering Perfect forecast, "
                                "the imbalances due to your submitted bid, and the feasible imbalances.",
                                target="fig_4", placement='left-end', style={'font-size': '0.7vw'}
                            ),
                            dcc.Loading(fig_day.fig_accum(), color=app.color_3, type='circle'),

                        ], id='fig_4', style={'width': '40vw', 'height': '25vw'}),
                    ], width=5, lg=5, md=5),
                ], style={'height': '25vw'}, justify="around",
            ),
    ...
    messages.mesag_confirm_nextday(),
...
def display_graph(nome):
    ...
    if (df.empty or len(df.columns) < 1 or clicks is None or P_value == 0):
        ...
        BB = act_prices_feas.iloc[0,:].values + (df.iloc[:, 1:5].sum(axis=1)) * pr[pr.columns[b2 + 1]].array
...
# ----------  SENDS TO PAGE 1 ----------- #
@app.callback(Output('link_cancel', 'disabled'),
              [
                  # Input('nextD_b', 'n_clicks'),
               Input('continue_button', 'n_clicks')])
def button_2(cont):

    # print('AQUI VAMOS:{}'.format(cont))
...
@app.callback(
    Output("modal_continue", "is_open"),
    [Input("continue_button", "n_clicks"),
     Input('nextD_b', 'n_clicks')],
    [State("modal_continue", "is_open")],
)
...
@app.callback(
    [Output('nextD_b', 'disabled'),
               Output('nextD_b', 'className')],
    [Input('graph_unbal', 'figure')]
)
def cont_next_day(fig):
